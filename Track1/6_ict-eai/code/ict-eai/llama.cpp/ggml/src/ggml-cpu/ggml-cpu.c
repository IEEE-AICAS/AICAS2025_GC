...
#define logf_fast(x) (log2f_approx(x) * 0.6931471806)
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                ...
                #if defined(__ARM_FEATURE_SVE)
                {
                    svbool_t pg = svwhilelt_b32(i00, ne00);  // 生成谓词
                    svfloat32_t sumv = svdup_n_f32(0.0f);    // 初始化向量和
                    while (svptest_any(svptrue_b32(), pg)) { // 循环直到处理完所有元素
                        svfloat32_t xv = svld1_f32(pg, x + i00); // 加载数据
                        sumv = svadd_f32_z(pg, sumv, xv);        // 累加
                        i00 += svcntw();                         // 增加向量长度
                        pg = svwhilelt_b32(i00, ne00);           // 更新谓词
                    }
                    sum = svaddv_f32(svptrue_b32(), sumv);       // 向量元素求和
                }
                ...
                #if defined(__ARM_FEATURE_SVE)
                {
                    svbool_t pg = svwhilelt_b32(i00, ne00);
                    svfloat32_t sumv = svdup_n_f32(0.0f);
                    svfloat32_t meanv = svdup_f32(mean);     // 广播 mean
                    while (svptest_any(svptrue_b32(), pg)) {
                        svfloat32_t xv = svld1_f32(pg, x + i00);    // 加载 x
                        svfloat32_t vv = svsub_f32_z(pg, xv, meanv); // v = x - mean
                        svst1_f32(pg, y + i00, vv);                 // 存储到 y
                        sumv = svmla_f32_z(pg, sumv, vv, vv);       // sum2 += v * v
                        i00 += svcntw();
                        pg = svwhilelt_b32(i00, ne00);
                    }
                    sum2 = svaddv_f32(svptrue_b32(), sumv);
                }
                #elif defined(__ARM_NEON)
                {
                    float32x4_t sumv = vdupq_n_f32(0.0f);
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                ...
                float sum = 0.0;
                int64_t i00 = 0;
                #if defined(__ARM_FEATURE_SVE)  // SVE 实现
                {
                    svbool_t pg = svwhilelt_b32(i00, ne00);  // 生成谓词
                    svfloat32_t sumv = svdup_n_f32(0.0f);    // 初始化向量和
                    while (svptest_any(svptrue_b32(), pg)) { // 循环直到处理完所有元素
                        svfloat32_t xv = svld1_f32(pg, x + i00); // 加载数据
                        sumv = svmla_f32_z(pg, sumv, xv, xv);    // sum += x * x
                        i00 += svcntw();                         // 增加向量长度
                        pg = svwhilelt_b32(i00, ne00);           // 更新谓词
                    }
                    sum = svaddv_f32(svptrue_b32(), sumv);       // 向量元素求和
                }
                #elif defined(__ARM_NEON)  // NEON 实现
                {
                    float32x4_t sumv = vdupq_n_f32(0.0f);
                    float32x4_t sumv1 = vdupq_n_f32(0.0f);
                    for(; i00 < ne00 - 8; i00 += 8){
