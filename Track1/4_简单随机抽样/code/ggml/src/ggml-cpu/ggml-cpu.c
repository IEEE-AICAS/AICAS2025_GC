...
//    *s = sumf;
    ...
    // svfloat16_t sum1 = svdup_n_f16(0.0);
    // svfloat16_t sum2 = svdup_n_f16(0.0);
    // svfloat16_t sum3 = svdup_n_f16(0.0);
    // svfloat16_t sum4 = svdup_n_f16(0.0);
    // svfloat16_t sum5 = svdup_n_f16(0.0);
    // svfloat16_t sum6 = svdup_n_f16(0.0);
    // for (int i = 0; i < np; i += 5*vl ) {
    //     svfloat16_t vec_x1 = svld1_f16(pg, (const float16_t *)&x[i]);
    //     svfloat16_t vec_y1 = svld1_f16(pg, (const float16_t *)&y[i]);
    //     sum1 = svmla_f16_m(pg, sum1, vec_x1, vec_y1);
        
    //     svfloat16_t vec_x2 = svld1_f16(pg, (const float16_t *)&x[i+vl]);
    //     svfloat16_t vec_y2 = svld1_f16(pg, (const float16_t *)&y[i+vl]);
    //     sum2 = svmla_f16_m(pg, sum2, vec_x2, vec_y2);

    //     svfloat16_t vec_x3 = svld1_f16(pg, (const float16_t *)&x[i+2*vl]);
    //     svfloat16_t vec_y3 = svld1_f16(pg, (const float16_t *)&y[i+2*vl]);
    //     sum3 = svmla_f16_m(pg, sum3, vec_x3, vec_y3);

    //     svfloat16_t vec_x4 = svld1_f16(pg, (const float16_t *)&x[i+3*vl]);
    //     svfloat16_t vec_y4 = svld1_f16(pg, (const float16_t *)&y[i+3*vl]);
    //     sum4 = svmla_f16_m(pg, sum4, vec_x4, vec_y4);

    //     svfloat16_t vec_x5 = svld1_f16(pg, (const float16_t *)&x[i+4*vl]);
    //     svfloat16_t vec_y5 = svld1_f16(pg, (const float16_t *)&y[i+4*vl]);
    //     sum5 = svmla_f16_m(pg, sum5, vec_x5, vec_y5);
        
    // }
    // // *s = 0;
    // // for (int i = np; i < n; ++i) {
    // //     *s += (ggml_float)(GGML_FP16_TO_FP32(x[i])*GGML_FP16_TO_FP32(y[i]));
    // // }

    // if(np + vl < n){
    //     svfloat16_t vec_x1 = svld1_f16(pg, (const float16_t *)&x[np]);
    //     svfloat16_t vec_y1 = svld1_f16(pg, (const float16_t *)&y[np]);
    //     sum1 = svmla_f16_m(pg, sum1, vec_x1, vec_y1);
        
    //     // svfloat16_t vec_x2 = svld1_f16(pg, (const float16_t *)&x[np+vl]);
    //     // svfloat16_t vec_y2 = svld1_f16(pg, (const float16_t *)&y[np+vl]);
    //     // sum2 = svmla_f16_m(pg, sum2, vec_x2, vec_y2);

    //     np += vl;
    // }

    // svbool_t pg1 = svwhilelt_b16(np, n);
    // svfloat16_t vec_x1 = svld1_f16(pg1, (const float16_t *)&x[np]);
    // svfloat16_t vec_y1 = svld1_f16(pg1, (const float16_t *)&y[np]);
    // sum1 = svmla_f16_m(pg, sum1, vec_x1, vec_y1); 

    // sum1 = svadd_f16_x(pg,svadd_f16_x(pg, sum1, sum2),svadd_f16_x(pg, sum3, sum4));
    // sum5 = svadd_f16_x(pg, sum5, sum6);
    // sum1 = svadd_f16_x(pg, sum1, sum5);

    // *s = svaddv_f16(pg, sum1);
    ...
    svfloat16_t vec_x1 = svld1_f16(pg1, (const float16_t *)&x[np]);
    svfloat16_t vec_y1 = svld1_f16(pg1, (const float16_t *)&y[np]);
    sum1 = svmla_f16_m(pg1, sum1, vec_x1, vec_y1);    
    *s = svaddv_f16(pg,svadd_f16_x(svptrue_b16(),svadd_f16_x(pg, sum1, sum2),svadd_f16_x(pg, sum3, sum4)));
