...
from smoothquant.fake_quant import quantize_qwen2_QAT
...
class Sparse(autograd.Function):
    ...
    def forward(ctx, weight, N, M, decay = 0.0002):
        ...
        w_b = torch.ones(weight_temp.shape, device=weight_temp.device,dtype=output.dtype)
...
class SparseLinear(nn.Linear):
    ...
    def __init__(self, in_features: int, out_features: int, bias, N=2, M=2, decay = 0.0002, **kwargs):
        self.N = N
        self.M = M
        self.decay=decay
...
def convert_to_sparse_linear(model, N=2, M=2,decay=0.0002):
    """
    将模型中的nn.Linear层转换为SparseLinear层
    Args:
        model: 预训练模型
        N, M: SparseLinear的稀疏参数
    """
    for name, module in model.named_children():
        if isinstance(module, nn.Linear):
            # 创建新的SparseLinear层
            # print(f"convert {name} to SparseLinear, bias: {module.bias is not None}")
            ...
            # 复制权重和偏置
            sparse_linear.weight.data = module.weight.data.clone()
            if module.bias is not None:
                sparse_linear.bias.data = module.bias.data.clone()
            
            # 替换原始层
            setattr(model, name, sparse_linear)
        else:
            # 递归处理子模块
            convert_to_sparse_linear(module, N, M)
...
class ModelArguments:
    ...
    lora: bool = field(
        default=False,
        metadata={"help": "Whether to use lora model."},
    )
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        # )
        # path_to_data = '/lamport/makkapakka/lilinqi/glue/'+data_args.task_name
        # raw_datasets = load_dataset(path_to_data)
        ...
        # 创建新的raw_datasets
        # print(f"train_size: {len(combined_dataset['train'])}, val_size: {len(combined_dataset['test'])}")
    ...
    if model_args.lora:
        ...
        model: PeftModel | PeftMixedModel = get_peft_model(model, peft_config=lora_config)
        model.print_trainable_parameters()

    model.config.pad_token_id=tokenizer.pad_token_id
    N=32
    M=64
    # model=convert_to_sparse_linear(model, N=N, M=M)
    model=remove_linear_bias(model)
    act_scales_per_token=torch.load('/lamport/makkapakka/lilinqi/smoothquant/static_scale.pt')
    model=quantize_qwen2_QAT(model,act_scales_per_token)
    # exit(0)#测试输出用
    ...
    # print(f"padding:{padding}\t max_seq{max_seq_length}")
    
    def preprocess_function(examples):
        # Tokenize the texts
        sentence1 = examples['sentence1']
        sentence2 = examples['sentence2']
        args=f"Sentence 1: {sentence1} Sentence 2: {sentence2} Is the second sentence entailed by the first? (1 for yes, 0 for no) you can only output 1 or 0"
        # args = (
