...
module ram_bypass_core#(
    parameter DATA_WIDTH=512,
    parameter DATA_GROUP=1,
    parameter URAM_WIDTH=72,
    parameter URAM_DEPTH=4096,
    parameter URAM_NUM=8
)(
    ...
    wire [URAM_NUM*URAM_WIDTH/8-1:0] write_mask_full=
        {{(URAM_NUM*URAM_WIDTH/8-DATA_WIDTH/8){1'b0}}, write_mask};
    wire [URAM_NUM*URAM_WIDTH/8-1:0] read_mask_full=
        {{(URAM_NUM*URAM_WIDTH/8-DATA_WIDTH/8){1'b0}}, {(DATA_WIDTH/8){1'b1}}};
    assign read_data_512=read_data_full;
    assign read_data=read_data_512;
    ...
    generate
        for(i=0; i<URAM_NUM; i=i+1) begin: loop_ram
            ...
            u_uram_core (
                ...
                .ADDR_A(write_addr),                         // 23-bit input: Port A address
                .ADDR_B(read_addr),                         // 23-bit input: Port B address
