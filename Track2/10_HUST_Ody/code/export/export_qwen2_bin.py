...
# #裁剪量化
# def quantize_q80(w, group_size, clip_percentile=95):
#     """
#     Applies Q8_0 quantization with clipping.
#     - `clip_percentile` defines the percentage of values to retain before quantization.
#     - Uses symmetric quantization into int8, range [-127, 127].
#     """
#     assert w.numel() % group_size == 0
#     ori_shape = w.shape
#     w = w.float()  # Convert to float32
...
#     # **1️ 计算分位数阈值，进行裁剪**
#     clip_val = torch.quantile(torch.abs(w), clip_percentile / 100.0, dim=1, keepdim=True)
#     w_clipped = torch.clamp(w, -clip_val, clip_val)

#     # **2️ 计算量化 Scale**
#     wmax = torch.max(torch.abs(w_clipped), dim=1).values  # 获取裁剪后的最大值
#     scale = wmax / 127.0  # 计算量化比例因子

#     # **3️ 进行对称量化**
#     quant = w_clipped / scale[:, None]  # 量化到 [-127,127]
#     int8val = torch.round(quant).to(torch.int8)  # 取整并转换为 int8

#     # **4️ 反量化（仅用于计算误差，不参与实际推理）**
#     fp32val = (int8val.float() * scale[:, None]).view(-1)
#     fp32valr = fp32val.reshape(-1, group_size)
...
def legacy_export(model, filepath):
    ...
    print('shared_classifier = %s', shared_classifier)
...
def version2_export(model, filepath, group_size=64):
    ...
    # first let's write out all the params that we are keeping in fp32: the norms
    #serialize_fp32(out_file, model.tok_embeddings.weight)#token_embedding_table不用量化
    for layer in model.layers:  # attention norms
        serialize_fp32(out_file, layer.attention_norm.weight)
    for layer in model.layers:  # MLP norms
        serialize_fp32(out_file, layer.ffn_norm.weight)
    ...
    # version2_export 里，在所有权重写完后写入标志
    # flag = 0xDEADBEEF
    # out_file.write(struct.pack('I', flag))  # 添加 4 字节的标志
    # out_file.close()
    # print(f"✅ Model exported successfully with debug flag {hex(flag)} at the end.")
