...
void malloc_run_state(RunState *s, Config *p) {
  ...
  s->x = calloc(p->seq_len * p->dim, sizeof(float));
  s->xb = calloc(p->seq_len * p->dim, sizeof(float));
  s->xb2 = calloc(p->seq_len * p->dim, sizeof(float));
  s->hb = calloc(p->seq_len * p->hidden_dim, sizeof(float));
  s->hb2 = calloc(p->seq_len * p->hidden_dim, sizeof(float));
  malloc_quantized_active(&s->xq, p->seq_len, p->dim);
  malloc_quantized_active(&s->hq, p->seq_len, p->hidden_dim);
  // s->xq = (QuantizedTensor){.q = calloc(p->seq_len * p->dim, sizeof(int8_t)), .s = calloc(p->seq_len, sizeof(float))};
  // s->hq = (QuantizedTensor){.q = calloc(p->seq_len * p->hidden_dim, sizeof(int8_t)), .s = calloc(p->seq_len, sizeof(float))};
  s->q = calloc(p->seq_len * p->dim, sizeof(float));
  s->att = calloc(p->n_heads * p->seq_len * p->seq_len, sizeof(float));
  s->logits = calloc(p->vocab_size, sizeof(float));
  s->key_cache = calloc(p->n_layers * p->seq_len * kv_dim, sizeof(float));
  s->value_cache = calloc(p->n_layers * p->seq_len * kv_dim, sizeof(float));
  // s->cq = calloc(p->seq_len * p->hidden_dim, sizeof(int32_t));
  // int seq_len = p->seq_len < MAX_SEQ_LEN ? p->seq_len : MAX_SEQ_LEN;
  s->cq = malloc_quantized_result(p->seq_len * p->hidden_dim);
  s->cq2 = malloc_quantized_result(p->seq_len * p->hidden_dim);
  s->fa_s = calloc(p->n_heads * FA_Q_BLOCK_SIZE * FA_K_BLOCK_SIZE, sizeof(float));
  s->fa_o = calloc(p->dim * FA_Q_BLOCK_SIZE, sizeof(float));
  s->fa_pv = calloc(p->dim * FA_Q_BLOCK_SIZE, sizeof(float));
  // ensure all mallocs went fine
  if (!s->x || !s->xb || !s->xb2 || !s->hb || !s->hb2 || !s->q || !s->key_cache || !s->value_cache || !s->att || !s->logits || !s->xq.q || !s->xq.s || !s->hq.q || !s->hq.s || !s->cq || !s->cq2 || !s->fa_o || !s->fa_s) {
...
void free_run_state(RunState *s) {
  ...
  free(s->hb);
  free(s->hb2);
  free_quantized_active(&s->xq);
  free_quantized_active(&s->hq);
  // free(s->xq.q);
  // free(s->xq.s);
  // free(s->hq.q);
  // free(s->hq.s);
  ...
  free(s->value_cache);
  // free(s->cq);
  free_quantized_result(s->cq);
  free_quantized_result(s->cq2);
}

void memory_map_weights(TransformerWeights *w, Config *p, void *ptr, int shared_weights, bool bias) {
  ...
  if(bias){
    w->bq = fptr;
    fptr += n_layers * (p->n_heads * head_size);
  }

  // K bias
  if(bias){
    w->bk = fptr;
    fptr += n_layers * (p->n_kv_heads * head_size);
  }
  ...
  // embed_tokens
  w->q_tokens = init_quantized_weights(&ptr, 1, p->dim, p->vocab_size);
  // dequantize token embedding table
  w->token_embedding_table = malloc(p->vocab_size * p->dim * sizeof(float));
  dequantize_embedding_reshape(w->q_tokens, w->token_embedding_table, p->vocab_size, p->dim);

  // Q weight
  w->wq = init_quantized_weights(&ptr, n_layers, p->dim, (p->n_heads * head_size));

  // K weight
  w->wk = init_quantized_weights(&ptr, n_layers, p->dim, (p->n_kv_heads * head_size));

  // V weight
  w->wv = init_quantized_weights(&ptr, n_layers, p->dim, (p->n_kv_heads * head_size));

  // O weight
  w->wo = init_quantized_weights(&ptr, n_layers, (p->n_heads * head_size), p->dim);

  // mlp.gate_proj
  w->w1 = init_quantized_weights(&ptr, n_layers, p->dim, p->hidden_dim);

  // mlp.down_proj
  w->w2 = init_quantized_weights(&ptr, n_layers, p->hidden_dim, p->dim);

  // mlp.up_proj
  w->w3 = init_quantized_weights(&ptr, n_layers, p->dim, p->hidden_dim);

  w->wcls = shared_weights ? w->q_tokens : init_quantized_weights(&ptr, 1, p->dim, p->vocab_size);
}
...
void read_checkpoint(char *checkpoint, Config *config, TransformerWeights *weights, int *fd, float **data, ssize_t *file_size) {
  ...
  // printf("dim = %d, hidden_dim = %d, n_layers = %d, n_heads = %d, n_kv_heads = %d, vocab_size = %d, seq_len = %d, head_size = %d\n",
  //        config->dim, config->hidden_dim, config->n_layers, config->n_heads, config->n_kv_heads, config->vocab_size, config->seq_len, config->dim / config->n_heads);
  ...
  // assert(GS == WI_ELEM_PER_GRID * WO_ELEM_PER_GRID);
  ...
  *data = mmap(NULL, *file_size, PROT_READ, MAP_PRIVATE, *fd, 0);
  ...
  void *weights_ptr = ((char *)*data) + sizeof(Config) + sizeof(int);
  memory_map_weights(weights, config, weights_ptr, shared_weights, true);
  // print_weights(weights, config, shared_weights, true);
}
...
void build_transformer(Transformer *t, char *checkpoint_path){
  ...
  // create lookup tables for rope
  init_rope(t->config.dim / t->config.n_heads);
}
...
float *prefill(Transformer *transformer, int *prompt_tokens, int prompt_token_num, bool need_bias) {
  start_segment(PREFILL);
  ...
  // copy the token embeddings into x
  start_segment(PREFILL_MEMCPY_EMBED);
#pragma omp parallel for
  for (int pos = 0; pos < prompt_token_num; pos++) {
    float *content_row = w->token_embedding_table + prompt_tokens[pos] * dim;
    memcpy(x + pos * dim, content_row, dim * sizeof(*x));
  }
  end_segment(PREFILL_MEMCPY_EMBED);

  // forward all the layers
  for (unsigned long long l = 0; l < p->n_layers; l++) {

    mha_prefill(transformer, l, prompt_token_num, need_bias);

    mlp_prefill(transformer, l, prompt_token_num, need_bias);

  }
  // final rmsnorm
#pragma omp parallel for
  for (int pos = 0; pos < prompt_token_num; pos++) {
    rmsnorm(x + pos * dim, x + pos * dim, w->rms_final_weight, dim);
  }

  // // copy the final state of the prompt tokens into the state
  // memcpy(s->x, x + (prompt_token_num - 1) * dim, dim * sizeof(*x));

  // classify into logits
  start_segment(PREFILL_FINAL_LOGITS);
  quantize_decode_reshape(&s->xq, x + (prompt_token_num - 1) * dim, dim);
  gemv(s->cq, &s->xq, w->wcls, dim, p->vocab_size);
  dequantize_decode_reshape(s->logits, s->xq.s, (w->wcls)->s, s->cq, p->vocab_size);
  end_segment(PREFILL_FINAL_LOGITS);
  end_segment(PREFILL);
  return s->logits;
}

float *forward(Transformer *transformer, int token, int pos,bool need_bias) {
  start_segment(DECODE);
  // a few convenience variables
  Config *p = &transformer->config;
  TransformerWeights *w = &transformer->weights;
  RunState *s = &transformer->state;
  ...
  for (unsigned long long l = 0; l < p->n_layers; l++) {
    ...
    mha_decode(transformer, l, pos, need_bias);

    mlp_decode(transformer, l, pos, need_bias);
  ...
  quantize_decode_reshape(&s->xq, x, dim);
  gemv(s->cq, &s->xq, w->wcls, dim, p->vocab_size);
  dequantize_decode_reshape(s->logits, s->xq.s, (w->wcls)->s, s->cq, p->vocab_size);
  end_segment(DECODE);
  return s->logits;
          ...
          int steps)
{
  ...
  long prefill_time=0;
  start = time_in_ms();
  float *logits = prefill(transformer, prompt_tokens, prompt_token_num, true);
  prefill_time = time_in_ms() - start;
  start = time_in_ms();
  int next = sample(sampler, logits);
  int pos = prompt_token_num;
  // Inference loop
  ...
  while (pos < steps) 
  {
    ...
    fflush(stdout);
    logits = forward(transformer, next, pos, true);
    next = sample(sampler, logits);
    pos++;
  }
  *prefill_throughput = prompt_token_num / (double)(prefill_time) * 1000;
  ...
  fprintf(stderr, "prompt token num: %d\n", prompt_token_num);
  fprintf(stderr, "decode token num: %d\n", pos - prompt_token_num);
  free(prompt_tokens);
  free(decoded_string);
                                ...
                                bool show_print,int steps)
{
  ...
  if (pos > prompt_token_num) {
    ...
    if (show_print) {
      fprintf(stderr, "achieved prefill tok/s: %f\n", prompt_token_num / (double)(prefill_time) * 1000);
