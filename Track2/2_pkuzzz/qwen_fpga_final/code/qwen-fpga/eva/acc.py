...
def invoked_c_func_manager(so_path):
    try:
        ...
        if hasattr(so_to_invoke, "xrt_setup"):
            ...
            xrt_setup()
        yield so_to_invoke.run_without_encode_decode
...
def infer(prompt,max_gen_step,hf_path,invoked_c_func):
...
def evaluate_model(dataset,hf_path,so_path):
    ...
    with Progress(
        ...
        BarColumn(),
        TaskProgressColumn(),
        TextColumn("{task.completed}/{task.total} tasks completed, {task.fields[wrong]} wrong"),
    ) as progress:
        
        task = progress.add_task("[bold green]Evaluation ", total=total_predictions, wrong=0)

        with invoked_c_func_manager(so_path) as invoked_c_func:
            for _, data in dataset.iterrows():
                ...
                out = infer(prompt, 1000,hf_path,invoked_c_func)
                # print(f"OUT={out} LABEL={label} PROMPT={prompt}")
                ...
                finish_cnt+=1
                progress.update(task, completed=finish_cnt,description=f"Evaluation ", wrong=finish_cnt-correct_predictions)
