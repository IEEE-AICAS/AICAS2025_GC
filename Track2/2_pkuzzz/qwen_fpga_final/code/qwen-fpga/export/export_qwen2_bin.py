...
def serialize_bf16(file, tensor):
    ...
    # Detach tensor, move to CPU, convert to bfloat16, and then convert to numpy array
    d = tensor.detach().cpu().view(-1).to(torch.bfloat16).view(torch.uint16).numpy()
    
    # bfloat16 uses 2 bytes per value, so we pack accordingly
    b = struct.pack(f'{len(d)}H', *d) 
    
    # Write the packed binary data to the file
    file.write(b)
...
def quantize_qb0_fake(w, group_size, b):
    """
    quantize a tensor into arbitrary effective bit b, but still store in int8
    """
    assert b <= 8
    assert w.numel() % group_size == 0
    ori_shape = w.shape
...
def quantize_q40_pack(w, group_size):
    """
    quantize a tensor into Q4_0, but store two in int8
    """
    assert w.numel() % group_size == 0
    ori_shape = w.shape
    w = w.float()  # convert to float32
    ...
    int4val = int4val.view(-1, 2)
    int8pack = torch.bitwise_and(int4val[:, 0], torch.tensor(0xf, dtype=torch.int8)) + int4val[:, 1] * torch.tensor(16, dtype=torch.int8)
    return int8pack, scale, maxerr


def quantize_q40_reshape(w, wi_grid_per_buf=8, wi_elem_per_grid=8, wo_grid_per_buf=8, wo_elem_per_grid=8):
    """
    quantize a tensor into Q4_0, and reshape the tensor for HW-friendly format
    """
    group_size = wi_elem_per_grid * wi_grid_per_buf  # group quantization size

    assert w.numel() % group_size == 0
    ori_shape = w.shape
    assert ori_shape[1] % (wi_elem_per_grid * wi_grid_per_buf) == 0
    assert ori_shape[0] % (wo_elem_per_grid * wo_grid_per_buf) == 0
    num_ibuf = ori_shape[1] // (wi_elem_per_grid * wi_grid_per_buf)
    num_obuf = ori_shape[0] // (wo_elem_per_grid * wo_grid_per_buf)
    ...
    # reshape the tensor for HW-friendly format
    # FIXME: if you comment the following 4 lines, this function will be consistent with original q40 packing
    int4val = int4val.reshape(num_obuf, wo_grid_per_buf, wo_elem_per_grid, num_ibuf, wi_grid_per_buf, wi_elem_per_grid)
    int4val = int4val.permute(0, 3, 1, 4, 2, 5).contiguous()  # permute to -> (num_obuf, num_ibuf, wo_grid_per_buf, wi_grid_per_buf, wo_elem_per_grid, wi_elem_per_grid)
    # scale = scale.unsqueeze(-1).repeat(1, num_ibuf)
    # scale = scale.reshape(num_obuf, wo_grid_per_buf, wo_elem_per_grid, num_ibuf)
    # scale = scale.permute(0, 3, 1, 2).contiguous()  # permute to -> (num_obuf, num_ibuf, wo_grid_per_buf, wo_elem_per_grid)
...
def legacy_export_quant(model, filepath, b=8):
    ...
    group_size = 64
    # next write out the embedding weights
    q, s, err = quantize_qb0_fake(model.tok_embeddings.weight, group_size, b=b)
    serialize_int8(out_file, q)
    serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.attention.wq.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.attention.wk.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.attention.wv.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.attention.wo.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.feed_forward.w1.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.feed_forward.w2.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_qb0_fake(layer.feed_forward.w3.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    # final classifier weights
    if not shared_classifier:
        # serialize_fp32(out_file, model.output.weight)
        q, s, err = quantize_qb0_fake(model.output.weight, group_size, b=b)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
...
def legacy_export_quant_w4a8(model, filepath,
                             wi_grid_per_buf = 8,
                             wi_elem_per_grid = 8,
                             wo_grid_per_buf = 16,
                             wo_elem_per_grid = 8,
    ...
    assert group_size == wi_grid_per_buf * wi_elem_per_grid

    quantize_q40_reshape_wrapper = partial(
        quantize_q40_reshape, 
        wi_grid_per_buf=wi_grid_per_buf, wi_elem_per_grid=wi_elem_per_grid, wo_grid_per_buf=wo_grid_per_buf, wo_elem_per_grid=wo_elem_per_grid)

    # next write out the embedding weights
    q, s, err = quantize_q40_reshape_wrapper(model.tok_embeddings.weight)
    serialize_int8(out_file, q)
    serialize_fp32(out_file, s)
    cnt = 0
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.attention.wq.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.attention.wk.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.attention.wv.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.attention.wo.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.feed_forward.w1.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.feed_forward.w2.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    for layer in model.layers:
        q, s, err = quantize_q40_reshape_wrapper(layer.feed_forward.w3.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
    # final classifier weights
    if not shared_classifier:
        # serialize_fp32(out_file, model.output.weight)
        q, s, err = quantize_q40_reshape_wrapper(model.output.weight)
        serialize_int8(out_file, q)
        serialize_fp32(out_file, s)
...
def load_hf_model(model_path):
    ...
    for name, param in hf_dict.items():
    ...
    for layer in model.layers:
        ...
        # print(layer.feed_forward.w1.weight[0])
        # print(layer.feed_forward.w1.weight[895])
        ...
        # print(layer.feed_forward.w3.weight[0][0])
        # print(layer.feed_forward.w3.weight[4863][895])
...
def model_export(model, filepath, version, dtype=torch.float32, 
                 wi_grid_per_buf=8, wi_elem_per_grid=8, 
                 wo_grid_per_buf=16, wo_elem_per_grid=8):
    """
    Versions docs:
    ...
    if version == 0:
        ...
        legacy_export_quant(model, filepath, b=4)
    elif version == 5:
        legacy_export_quant_w4a8(model, filepath)
    elif version == 6:
        legacy_export_bf16(model, filepath)
    elif version == 7:
        legacy_export_quant_w4a8(model, filepath, 
                                 wi_grid_per_buf=wi_grid_per_buf, 
                                 wi_elem_per_grid=wi_elem_per_grid, 
                                 wo_grid_per_buf=wo_grid_per_buf, 
                                 wo_elem_per_grid=wo_elem_per_grid)
    elif version == -1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--version", default=3, type=int, help="the version to export with")
    parser.add_argument("--dtype", type=str, help="dtype of the model (fp16, fp32)", default="fp32")
    parser.add_argument("--wi_grid_per_buf", type=int, help="wi_grid_per_buf", default=8)
    parser.add_argument("--wi_elem_per_grid", type=int, help="wi_elem_per_grid", default=8)
    parser.add_argument("--wo_grid_per_buf", type=int, help="wo_grid_per_buf", default=16)
    parser.add_argument("--wo_elem_per_grid", type=int, help="wo_elem_per_grid", default=8)
    ...
    model_export(model, args.filepath, args.version, args.dtype, 
                 wi_grid_per_buf=args.wi_grid_per_buf, 
                 wi_elem_per_grid=args.wi_elem_per_grid, 
                 wo_grid_per_buf=args.wo_grid_per_buf, 
                 wo_elem_per_grid=args.wo_elem_per_grid)
