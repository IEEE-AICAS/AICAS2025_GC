...
typedef struct {
    ...
    XW4a8_gemm_uio_map maps[ MAX_UIO_MAPS ];
} XW4a8_gemm_uio_info;

/***************** Variable Definitions **************************************/
static XW4a8_gemm_uio_info uio_info;
...
static int uio_info_read_name(XW4a8_gemm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XW4a8_gemm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XW4a8_gemm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XW4a8_gemm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XW4a8_gemm_Initialize(XW4a8_gemm *InstancePtr, const char* InstanceName) {
	XW4a8_gemm_uio_info *InfoPtr = &uio_info;
