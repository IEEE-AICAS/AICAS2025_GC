...
int main(int argc, char** argv)
{
    ...
    //
    kernel = clCreateKernel(program, "w4a8_gemm", &err);
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_axi00_ptr0_output[i]) {
            printf("ERROR in w4a8_gemm::m00_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_axi00_ptr0_output[i], h_axi00_ptr0_output[i]);
            check_status = 1;
        }
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_axi01_ptr0_output[i]) {
            printf("ERROR in w4a8_gemm::m01_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_axi01_ptr0_output[i], h_axi01_ptr0_output[i]);
            check_status = 1;
        }
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_axi02_ptr0_output[i]) {
            printf("ERROR in w4a8_gemm::m02_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_axi02_ptr0_output[i], h_axi02_ptr0_output[i]);
            check_status = 1;
        }
