...
void compute_mac_sub(
    ...
    lp_cmp_sub_h:for (int m = 0; m < _MACRO_H/UR_PRA; m++) {
        ...
        // 阶段2：批量计算scaled_xi
        float scaled_xi[_MACRO_W];
        #pragma HLS ARRAY_PARTITION variable=scaled_xi complete
        lp_precompute:for (int j = 0; j < _MACRO_W; j++) {
            #pragma HLS UNROLL
            scaled_xi[j] = xi_val * qscale[j];
        }
        ...
        lp_cmp_mac_a:for (int j = 0; j < _MACRO_W; j++) {
            #pragma HLS UNROLL
            ap_int<5> w_ubias = w_uint4[j] - qzeros[j];
            pout_temp[j] += scaled_xi[j] * (float)w_ubias;
        }
    }
}
...
void compute_mac(
    ...
    lp_cmp_mac_cplocal:for (int j = 0; j < _MACRO_W; j++) {
        #pragma HLS PIPELINE II=1
        #pragma HLS LATENCY max=2  // 约束关键路径
...
void Macro_MAC_Acc4_top(
    ...
    // #pragma HLS INTERFACE m_axi port=xi   depth=1216 offset=slave bundle=gmem0 name=xi_r
    ...
    #pragma HLS ARRAY_PARTITION variable=pout_local0 complete
    #pragma HLS ARRAY_PARTITION variable=pout_local1 complete
    #pragma HLS ARRAY_PARTITION variable=pout_local2 complete
    #pragma HLS ARRAY_PARTITION variable=pout_local3 complete
