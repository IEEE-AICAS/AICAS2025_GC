...
void matmul_by_kv260(float* xout, QuantizedTensor *x, QuantizedTensor *w, int n, int d) {
  // W (d,n) @ x (n,) -> xout (d,)
  // by far the most amount of time is spent inside this little function
  // inputs to this function are both quantized
  ...
  for (i = 896; i < input_size; i++) {
      buffer_0[i] = w->q[i-896];
  }
  ...
  for (i = 0; i < d; i++) {
      ...
      int j;
      for (j = 0; j <= n - GS; j += GS) {
          rd_data = buffer_1[14*i + j/GS];
          val += ((float) rd_data) * w->s[(in + j) / GS] * x->s[j / GS];
          rd_data = 0;
      }
      xout[i] = val;
  }
...
float *forward(Transformer *transformer, int token, int pos,bool need_bias, int prompt_token_num) {
  ...
  for (unsigned long long l = 0; l < p->n_layers; l++) {
    ...
    if (pos < 1){
      matmul_by_kv260(s->q, &s->xq, w->wq + l, dim, dim);
      // matmul_by_kv260(s->k, &s->xq, w->wk + l, dim, kv_dim);
      // matmul_by_kv260(s->v, &s->xq, w->wv + l, dim, kv_dim);
    }
    else{
          ...
          int steps)
{
  ...
  while (pos < steps) 
  {
    ...
    float *logits = forward(transformer, token, pos,true,prompt_token_num);
