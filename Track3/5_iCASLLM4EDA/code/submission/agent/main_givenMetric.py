...
def test(num,Gain,GBW,SR,PM,IDC):
    # Define the agent
    ...
    if 2*GBW >55 and 2*GBW <81:
        result_list=calculate_ratio(2*GBW,0.5,2*GBW/1.2,38,28,0.22,20)
    elif 2*GBW>=81:
        result_list=calculate_ratio(85,0.5,2*GBW/1.8,38,28,0.22,20)
    else:
        result_list=calculate_ratio(2.5*SR,0.5,2*GBW,38,28,0.22,20)
...
def evaluate_metrics(sim_results, expected_results,not_default):
    evaluation_matrix = [False]*len(sim_results)
    print(sim_results, expected_results)
    for idx in range(len(sim_results)):
        if not_default[idx]:
            if idx != 4:
                if float(sim_results[idx]) > expected_results[idx]*0.9 and float(sim_results[idx]) < expected_results[idx]*1.1:
                    evaluation_matrix[idx] = True
            else:
                if float(sim_results[idx]) < expected_results[idx]*1.1 and float(sim_results[idx]) > expected_results[idx]*0.9:
                    evaluation_matrix[idx] = True
        else:
            evaluation_matrix[idx] = True
...
def ResultsExtraction(AC_results_dir, SR_results_dir, idc_results_dir):
    all_results = []
    ...
    return all_results
def test_flow(user_inputs,not_default):
    ...
    PM = 0
    IDC = 0
    for indicator, value in user_inputs.items():
            if indicator == "Gain":
                Gain = value
            if indicator == "GBW":
                GBW = value
    ...
    expected_results = [Gain, GBW, PM, SR, IDC]
    metrix = ['gain_value', 'gbw_value', 'pm_value', 'sr_value', 'idc_value']
    while (not Satisfied) and (iteration_time < 20):
        # ngspice simulation
        # ipdb.set_trace
        ...
        AC_results_dir = f'/root/submission/agent/{corner}_AC_results.txt'
        SR_results_dir = f'/root/submission/agent/{corner}_SR.txt'
        idc_results_dir = f'/root/submission/agent/{corner}_Idc.txt'
        ...
        sim_results = ResultsExtraction(AC_results_dir, SR_results_dir, idc_results_dir)
        ...
        with open(f'{corner}_iteration_records.txt', 'a') as file:
            ...
            print_to_file('Gain:', sim_results[0], 'GBW:', sim_results[1], 'PM:', sim_results[2],  'SR:', sim_results[3], 'IDC:', sim_results[4])
            print_to_file("------------------------")
            # Finetune Logic
            evaluation_result_metrix = evaluate_metrics(sim_results, expected_results,not_default)
            print_to_file("------------------------")
            print_to_file("Evaluation Result Metrix:")
            print_to_file('Gain:', evaluation_result_metrix[0], 'GBW:', evaluation_result_metrix[1], 'PM:', evaluation_result_metrix[2],'SR:', evaluation_result_metrix[3], 'IDC:', evaluation_result_metrix[4])
            print_to_file("------------------------")
...
if __name__ == '__main__':
    ...
    #create(model='qwenmath7b_modified', from_= 'mightykatun/qwen2.5-math:1.5b', system=TIR_SYSTEM)
    '''
    query = read_deepseek_instruction_prompt('../prompt_deepseek_pythonver/read_instruct.txt')
    print(query)
    os.system(f"ollama run tripplyons/r1-distill-qwen-7b '{query}' | tee instruction.txt")
    with open(f'instruction.txt', 'r', encoding='utf-8') as file:
        response_str = file.read()
    #print(response_str)
    '''
    ...
    for indicator in default_values.keys():
        user_input = input(f"Please set {indicator}（Please make sure its unit is：{units[indicator]}，if this metric is not to be set, directly press ENTER to skip）：")
        if user_input.strip():  
            try:
                user_inputs[indicator] = float(user_input) 
                not_default.append(1)
            except ValueError:
                print(f"The input {indicator} is invalid，please retry。")
                flag=1
                break
                #user_inputs[indicator] = default_values[indicator]
        else: 
            user_inputs[indicator] = default_values[indicator]
            not_default.append(0)
            print(f"The metric{indicator} is not set.")

    if flag:
        raise ValueError("\nSetting configurations not done, due to the invalid input. Please input numbers.")
    else:
        print("\nSetting configurations done")
        #for indicator, value in user_inputs.items():
            #print(f"{indicator}(unit:{units[indicator]}:{value}")
        test_flow(user_inputs,not_default)
        ...
        os.system('cp /root/submission/final_netlist2/AMP.cir /root/submission/agent/AMP.cir')
    #for i in range (0,3):
        #execute_iteration_until_not_none(iteration)
